...
export function NeedHelp({ ...props }: HTMLProps<HTMLElement>) {
  ...
  return (
    <motion.section
      ...
      {...(props as any)}
      className={` text-white relative md:h-screen min-h-screen w-full z-[2]  bg-transparent justify-start ${getResponsiveClasses()}`}
    >
      <div className="flex flex-col justify-between h-full py-[10vh]">
        <div className="flex mb-2 flex-col md:flex-row justify-between gap-2 w-full">
          <div className="md:basis-1/2 items-center shrink-0">
          ...
          <div className="md:basis-96 shrink-0">
        ...
        <div className="relative rounded-3xl gap-y-3 px-6 py-9 flex flex-col md:flex-row justify-between items-center border-white/65 border-2 pointer-events-none">
          <div className="h-[244px] hidden md:block w-full -top-[244px] absolute left-1/2 -translate-x-1/2 overflow-hidden">
            <Bot3D scale={1.5} />
          </div>
