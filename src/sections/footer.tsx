...
export function Footer({}: HTMLProps<HTMLElement>) {
  return (
    <section className={`${getResponsiveClasses()} relative z-10 py-5`}>
      <div className="flex justify-between gap-5 gap-y-5 items-start pb-3 border-b border-white/45 flex-wrap text-white">
        ...
        <div className="min-w-fit flex  gap-1 items-center">
          {[1, 2, 3].map((v) => (
            <img src="/footer_img.svg" className="w-16 h-16" key={v}></img>
          ))}
        </div>
      </div>
      <div className="py-3 flex flex-col gap-y-5 md:flex-row items-center justify-between">
        <div className="flex flex-col items-center sm:flex-row justify-center gap-1 sm:gap-7">
        ...
        <div className="flex flex-1 flex-row-reverse items-center gap-3  text-primary">
          <a href="https://x.com/vconektofficial">
            <svg
              width="21"
              height="20"
              className="hover:scale-110 text-base transition-all duration-150 cursor-pointer"
              viewBox="0 0 21 20"
              fill="none"
